...
function Shader(gl, sdr_id){
	this.id = (function(){
		...
		if ( !shader )
			throw Error("{Shader}:: elemento HTML: " + sdr_id + " ṇo foi encontrado");
		...
		id = gl.createShader(function()	{
			if ( shader.type === "x-shader/x-vertex" ) {
				...
				throw Error("{Shader}:: ID = " + sdr_id + ". O tipo " + shader.type + " ṇo ̩ suportado");
			}
			return shader_type;
		}() );
		...
		if ( !gl.getShaderParameter(id, gl.COMPILE_STATUS) ){
			var msg = "Erro de compila̤̣o no " + 
			(function(){
				switch(shader_type){
